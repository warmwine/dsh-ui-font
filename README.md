...
- **🎯 选取区域（所见即所得）**：设置页按钮或**全局快捷键 Ctrl+Shift+F** 开启（设置
  面板会遮挡界面——快捷键可在关闭面板后选取任意区域）。开启后鼠标变准星，悬停
  任意区块即高亮并显示归属插件与当前字号；点击后就地弹出 −/+ 控制条，直接调整
  该区域的字号偏移，即时生效。ESC / 右键取消。"这块界面归哪个设置管"从此不是问题
...
4. 选取器：进入时对所有插件样式表建"类名 → 归属插件"索引，悬停时 O(1) 查表
   定位归属；命中令牌驱动的区块时提示"全局偏移"，xterm 终端提示"内置字体"
   （iframe 内部的内容区不参与拾取）
