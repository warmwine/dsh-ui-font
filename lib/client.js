    ...
    factory: (require) => {
        ...
        function loadSettings() {
            try {
                ...
                if (raw === null) return { uiFont: DEFAULTS.uiFont, codeFont: DEFAULTS.codeFont, delta: 3, perPlugin: {}, perRule: {}, perToken: {}, hotkey: "", hotkeyUp: "", hotkeyDown: "" };
                ...
                return { uiFont: DEFAULTS.uiFont, codeFont: DEFAULTS.codeFont, delta: 3, perPlugin: {}, perRule: {}, perToken: {}, hotkey: "", hotkeyUp: "", hotkeyDown: "" };
            }
        }
        ...
        function FontSettingsPage() {
            ...
            const update = (patch) => {
                /* synchronous write first (belt), then React state (suspend):
                   a rejected updater would otherwise silently drop the change —
                   the preset-button bug: click stored nothing. */
                try {
                    const direct = Object.assign(loadSettings(), patch);
                    saveSettings(direct);
                    reapply();
                } catch (e) { /* storage unavailable — state path still runs */ }
                setS((prev) => {
