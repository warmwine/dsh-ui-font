...
window.__ModuleLoader__.load({
    ...
    factory: (require) => {
        ...
        /** Enter pick mode. onPick(result) fires on click with
            { plugin, pluginName, sizePx, isXterm, x, y } and the session has
            already ended. Returns an exit function (idempotent). */
        function enterPickMode(onPick) {
            ...
            const onPickClick = (e) => {
                ...
                const result = {
                    plugin: plugin,
                    pluginName: plugin === null ? null : plugin,
                    sizePx: size,
                    isXterm: t.closest(".xterm") !== null,
                    x: e.clientX,
                    y: e.clientY
                };
                exitPickMode();
        ...
        function FontSettingsPage() {
            ...
            /* Keep this page in sync with changes made outside React state —
               e.g. the pick-mode floating control writes settings directly. */
            const [pickResult, setPickResult] = React.useState(null);
            ...
            /* Pick mode: crosshair over the whole page; on click show an
               in-place −/+ control bound to the owning plugin (or global). */
            const startPick = () => {
                setPicking(true);
                setPickResult(null);
                enterPickMode((result) => {
                    setPicking(false);
                    if (result === null) return;
                    setPickResult({
                        plugin: result.plugin,
                        label: result.plugin === null ? "主界面（全局偏移）"
                            : result.plugin === "shell" ? "主界面外壳"
                            : prettifyId(result.plugin),
                        isXterm: result.isXterm,
                        sizePx: result.sizePx,
                        x: Math.min(result.x, window.innerWidth - 240),
                        y: Math.min(result.y + 12, window.innerHeight - 90),
                        current: result.plugin === null
                            ? (loadSettings().delta)
                            : (typeof loadSettings().perPlugin[result.plugin] === "number" ? loadSettings().perPlugin[result.plugin] : 0)
                    });
                });
            };
            const bumpPicked = (d) => {
                setPickResult((prev) => {
                    ...
                    const nextVal = Math.max(-6, Math.min(20, prev.current + d));
                    const s2 = loadSettings();
                    if (prev.plugin === null) s2.delta = nextVal;
                    else {
                        s2.perPlugin = Object.assign({}, s2.perPlugin);
                        if (nextVal === 0) delete s2.perPlugin[prev.plugin];
                        else s2.perPlugin[prev.plugin] = nextVal;
                    }
                    saveSettings(s2);
                    reapply();
                    window.dispatchEvent(new Event("dsh-ui-font:changed"));
                    return Object.assign({}, prev, { current: nextVal });
                });
            };
            const pickControl = pickResult === null ? null : React.createElement("div", {
                style: {
                    position: "fixed", left: pickResult.x, top: pickResult.y, zIndex: 2147483646,
                    background: "#111827", color: "#f9fafb", borderRadius: 10, padding: "8px 10px",
                    boxShadow: "0 4px 16px rgba(0,0,0,.4)", display: "flex", alignItems: "center", gap: 10,
                    fontSize: 13
                }
            },
                React.createElement("div", { style: { maxWidth: 180, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" } },
                    pickResult.label,
                    React.createElement("div", { style: { fontSize: 11, opacity: 0.7 } },
                        pickResult.isXterm ? "终端字体由 dsh-ssh 内置，不受控制" : "当前字号 " + pickResult.sizePx)),
                React.createElement("button", { onClick: () => bumpPicked(-1), style: { width: 30, height: 30, fontSize: 16 } }, "−"),
                React.createElement("span", { style: { minWidth: 34, textAlign: "center", fontVariantNumeric: "tabular-nums" } },
                    (pickResult.current > 0 ? "+" : "") + pickResult.current),
                React.createElement("button", { onClick: () => bumpPicked(1), style: { width: 30, height: 30, fontSize: 16 } }, "+"),
                React.createElement("button", { onClick: () => setPickResult(null), style: { height: 30 } }, "完成"));
            ...
            return React.createElement("div", { style: { display: "flex", flexDirection: "column", gap: 4 } },
                ...
                React.createElement("p", { style: { color: "rgba(128,128,128,.9)", margin: "4px 0 8px" } },
                    "不知道某块界面对应哪一项？点「选取区域」，再用准星点它——就地加减字号，所见即所得。"),
                React.createElement("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "center", margin: "8px 0 4px", gap: 8 } },
                    ...
                    React.createElement("div", { style: { display: "flex", gap: 6 } },
                        React.createElement("button", {
                            onClick: picking ? exitPickMode : startPick,
                            title: picking ? "ESC 或右键退出" : "点击后用准星在页面上点选任意区块",
                            style: picking ? { outline: "2px solid #e11d48" } : undefined
                        }, picking ? "选取中…（ESC 取消）" : "🎯 选取区域"),
