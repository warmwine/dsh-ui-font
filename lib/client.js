...
window.__ModuleLoader__.load({
    ...
    factory: (require) => {
        ...
        /** Which plugin styles the element (or an ancestor up the tree)?
            null = token-driven (shell), no plugin class nearby. Walks up to 10
            ancestors — deep panel nesting (portal wrappers, layout stacks) is
            normal in this GUI. */
        function pluginForElement(el, index) {
            ...
            let node = el;
            for (let i = 0; i < 11 && node !== null; i++) {
                const hit = check(node);
        ...
        function enterPickMode() {
            ...
            const onMove = (e) => {
                ...
                /* lazily-injected stylesheets: when this element's classes are
                   unknown to the index, one refresh picks up new sheets (the
                   cheap scanAll is incremental — already-seen sheets skipped). */
                let unknown = false;
        ...
        function FontSettingsPage() {
            ...
            const comboKbd = s.hotkey === ""
                ? null
                : React.createElement("span", { style: { display: "inline-flex", gap: 4 } },
                    s.hotkey.split("+").map((k, i) => React.createElement("kbd", { key: i, style: kbdStyle }, k)));
            const hotkeyControl = React.createElement(React.Fragment, null,
                s.hotkey !== "" && recording !== true ? comboKbd : null,
                s.hotkey === "" && recording !== true
                    ? React.createElement("span", { style: { color: UI.warn, fontSize: 12 } }, "未设置") : null,
                recording
                ...
                React.createElement("button", {
                    ...
                    style: btnStyle(recording ? "danger" : s.hotkey === "" ? "danger" : undefined),
                    title: "点击后按下组合键（需含 Ctrl/Alt/Meta 之一；ESC 取消）"
                }, recording ? "取消" : s.hotkey === "" ? "录制" : "修改"),
                s.hotkey !== "" && recording !== true
                    ? React.createElement("button", {
            ...
            return React.createElement("div", { style: { display: "flex", flexDirection: "column", color: UI.text } },
                ...
                React.createElement("p", { style: { color: UI.text2, margin: "2px 2px 0", fontSize: 12.5 } },
                    "每个区域的实际缩放 = 全局偏移 + 个体偏移（下方逐项叠加）。快捷键随时召唤「准星选取」——设置面板会挡住界面，关掉设置再按它选取任意区域，ESC / 右键退出。"),
